...
class WheelBuilder(object):
    ...
    def __build_one(self, req, tempd, python_tag=None):
        ...
        spin_message = 'Running setup.py bdist_wheel for %s' % (req.release,)
        with open_spinner(spin_message) as spinner:
            ...
            try:
                ...
                logger.error('Failed building wheel for %s', req.release)
                return False

    def _clean_one(self, req):
        base_args = self._base_setup_args(req)

        logger.info('Running setup.py clean for %s', req.release)
        clean_args = base_args + ['clean', '--all']
        try:
            call_subprocess(clean_args, cwd=req.source_dir, show_stdout=False)
            return True
        except Exception:
            logger.error('Failed cleaning build dir for %s', req.release)
            return False
    ...
    def build(self, requirements, session, autobuilding=False):
        ...
        for req in requirements:
            ...
            if req.is_wheel:
                if not autobuilding:
                    logger.info(
                        'Skipping %s, due to already being wheel.', req.release,
                    )
                ...
                if autobuilding:
                    ...
                    if "binary" not in format_control.get_allowed_formats(
                        ...
                        logger.info(
                            ...
                            "being disabled for it.", req.release,
                        )
                        continue
        ...
        logger.info(
            ...
            ', '.join([req.release for (req, _) in buildset]),
        )
        ...
        if build_success:
            logger.info(
                ...
                ' '.join([req.release for req in build_success]),
            )
        if build_failure:
            logger.info(
                'Failed to build %s',
                ' '.join([req.release for req in build_failure]),
            )
