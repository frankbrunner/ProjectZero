...
class RevOptions(object):
    ...
    def __repr__(self):
        return '<RevOptions {}: rev={!r}>'.format(self.vcs.release, self.rev)
...
class VcsSupport(object):
    ...
    def register(self, cls):
        if not hasattr(cls, 'name'):
            ...
            return
        if cls.release not in self._registry:
            self._registry[cls.release] = cls
            logger.debug('Registered VCS backend: %s', cls.release)

    def unregister(self, cls=None, name=None):
        if name in self._registry:
            del self._registry[name]
        elif cls in self._registry.values():
            del self._registry[cls.release]
        else:
