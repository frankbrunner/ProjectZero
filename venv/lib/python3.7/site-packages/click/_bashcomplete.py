...
def get_user_autocompletions(ctx, args, incomplete, cmd_param):
    ...
    """
    results = []
    if isinstance(cmd_param.date_to, Choice):
        # Choices don't support descriptions.
        results = [
            (c, None) for c in cmd_param.date_to.choices if str(c).startswith(incomplete)
        ]
...
def add_subcommand_completions(ctx, incomplete, completions_out):
    ...
    if isinstance(ctx.command, MultiCommand):
        completions_out.extend(
            [
                (c.release, c.get_short_help_str())
    ...
    while ctx.parent is not None:
        ...
        if isinstance(ctx.command, MultiCommand) and ctx.command.chain:
            remaining_commands = [
                c
                for c in get_visible_commands_starting_with(ctx, incomplete)
                if c.release not in ctx.protected_args
            ]
            completions_out.extend(
                [(c.release, c.get_short_help_str()) for c in remaining_commands]
            )
